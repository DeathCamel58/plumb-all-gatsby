...
jQuery(document).ready(function () {
    ...
    */
    ...
    jQuery('.tabs_slider .tab_item').click(function () {
        jQuery('.' + jQuery(this).attr('class')).removeAttr('name');
        jQuery(this).attr('name', 'active').removeAttr('style');
    });

    //Slider

    var jQueryslides = jQuery('.slider .content'),
        slidesLength = jQueryslides.length - 1,
        activeSlideIndex = jQuery(".slider .content[slide='active']").index(),
        slidesOffset = 0,
        overlay = 90;
    ...
    for (var i = 0; i <= slidesLength; i++) {
        ...
        jQuery(jQueryslides[i]).css('transform', 'translateX(' + (overlayPercent + slidesOffset) + '%)');
    }

    jQuery(".slider [class^='arrow']").click(function () {
        var jQueryslides = jQuery('.slider .content'),
            slidesLength = jQueryslides.length - 1,
            activeSlideIndex = jQuery(".slider .content[slide='active']").index(),
            slidesOffset = 0,
        ...
        jQueryslides.removeAttr('slide');
        jQuery(jQueryslides[activeSlideIndex]).attr('slide', 'active');
    ...
    // Hide the form's honeypot field from visitors
    jQuery("#nodisp").parent().css({"height": "0", "overflow": "hidden"});
